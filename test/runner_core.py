...
def find_dll():
    ...
    # warn
    print("KTX DLL 파일을 찾을 수 없습니다. 현재 디렉토리에서 'ktxdll.dll' 또는 '../build/vcpp_core.dll'을 확인하세요.")
    return None

DLL_PATH = find_dll()
CORE_DLL = None  # ctypes로 로드된 DLL 객체를 저장할 변수
CORE_KTX_FUNC = None  # DLL 내의 ktx_main 함수 포인터를 저장할 변수
CORE_FBX_FUNC = None  # DLL 내의 fbx_main 함수 포인터를 저장할 변수
CORE_OIDN_FUNC = None  # DLL 내의 oidn_main 함수 포인터를 저장할 변수
...
def parse_cli_string(command_string):
    ...
    try:
        ...
        return [] # 또는 더 엄격한 오류 처리를 원한다면 예외를 다시 발생시킬 수 있습니다.

def init_dll():
    """
    설정된 DLL_PATH를 사용하여 KTX DLL을 로드하고 ktx_main 함수를 준비합니다.
    이 함수는 스크립트 시작 시 또는 set_dll_path 호출 시 실행됩니다.
    """
    global DLL_PATH, CORE_DLL, CORE_KTX_FUNC
    ...
    try:
        ...
        print("DLL 파일이 존재하고 시스템의 PATH 또는 스크립트 디렉토리에 있는지 확인하세요.")
        exit(1)

    # 2. ktx_main 함수의 시그니처를 정의합니다.
    # C/C++에서의 함수 선언: int ktx_main(int argc, char* argv[])
    try:
        CORE_KTX_FUNC = CORE_DLL.vcpp_ktx
        CORE_KTX_FUNC.argtypes = [ctypes.c_int, ctypes.POINTER(ctypes.c_char_p)]
        CORE_KTX_FUNC.restype = ctypes.c_int
    except AttributeError:
        print(f"오류: DLL '{DLL_PATH}'에서 'ktx_main' 함수를 찾을 수 없습니다.")
        print("함수가 올바르게 익스포트되었는지 확인하세요 (예: __declspec(dllexport) 및 extern \"C\" 사용).")
        exit(1)

    try:
        CORE_FBX_FUNC = CORE_DLL.vcpp_fbx
        CORE_FBX_FUNC.argtypes = [ctypes.c_int, ctypes.POINTER(ctypes.c_char_p)]
    ...
    try:
        ...
        CORE_OIDN_FUNC.argtypes = [ctypes.c_int, ctypes.POINTER(ctypes.c_char_p)]
...
def pyktx(cmd:str):
    ...
    Returns:
        int: ktx_main 함수에서 반환된 정수형 반환 코드.
    """
    return pyktx_list(parse_cli_string(cmd))

def pyktx_list(cmd_list:list[str]):
    """
    이미 파싱된 명령줄 인자 리스트를 받아 DLL의 ktx_main 함수를 호출합니다.
    ...
    if CORE_KTX_FUNC is None:
        print("오류: KTX DLL 또는 ktx_main 함수가 초기화되지 않았습니다. init_dll()을 먼저 호출하세요.")
    ...
    # print(f"argc={argc}, argv={[arg for arg in cmd_list]}로 ktx_main 호출 중")
    
    # 함수를 호출합니다.
    return_code = CORE_KTX_FUNC(argc, argv_c)
    
    # print(f"ktx_main 반환 값: {return_code}")
    return return_code